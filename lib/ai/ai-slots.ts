...
export const AI_SLOT_DESCRIPTIONS: Record<AiModelSlot, string> = {
  creative: 'Long-form bio, backstory, skill distribution analysis, portrait prompts.',
  simple: 'Names, short traits, quick JSON helpers.',
  vision: 'Describe portraits, crop analysis, feature extraction.',
  image: 'Portrait and expressive image generation.',
};
...
export const readStoredProviderKey = (provider: AiProviderId): string => {
  ...
  if (provider === 'xai-oauth') return ''; // OAuth never uses the key vault field
  try {
...
export const writeStoredProviderKey = (provider: AiProviderId, key: string) => {
  if (typeof window === 'undefined' || provider === 'xai-oauth') return;

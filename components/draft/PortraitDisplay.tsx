...
const PortraitSelector: React.FC<{ isSelected: boolean; onClick: (e: React.MouseEvent) => void }> = ({ isSelected, onClick }) => (
    <button
        onClick={onClick}
        className="absolute top-2 left-2 z-30 w-8 h-8 bg-black/60 rounded-full flex items-center justify-center border-2 border-neutral-400 hover:border-primary transition-all"
    ...
    >
        {isSelected ? (
            ...
            <div className="w-5 h-5 rounded-full border-2 border-neutral-300 bg-black/30"></div>
        )}
    </button>
);
...
export const PortraitDisplay: React.FC<PortraitDisplayProps> = ({
    ...
    return (
        <div className={`w-full max-w-[288px] mx-auto ${aspectRatio} bg-cream-100 rounded-lg p-2 border border-border flex justify-center items-center relative shadow-inner`}>
            {isLoading ? (
                ...
                <div className="text-center text-danger-700 p-4">
                ...
                <div className="absolute inset-2">
                     ...
                     {showDownload && (
                        <button
                            ...
                            className="absolute top-2 right-2 z-20 p-2 bg-black/50 rounded-full text-white hover:bg-primary/80 hover:text-primary-foreground transition-all duration-200"
                ...
                <div className="text-center text-muted-foreground">
                    <PortraitIcon className="h-16 w-16 mx-auto text-border mb-2" />

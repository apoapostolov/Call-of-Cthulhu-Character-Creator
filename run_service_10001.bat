...
set "TASK_NAME=CoC_DholeHouse_Dev_10001"
...
REM Create a per-user scheduled task that starts the dev server on port 10001 at logon
set "TR_CMD=cmd /c cd /d \"%~dp0\" ^&^& npm run dev -- --port 10001 --host --strictPort"
...
if %ERRORLEVEL% EQU 0 (
  echo Service task "%TASK_NAME%" created and started on port 10001.
